...
Axis mainAxisForTrackType(TrackType type) => type == TrackType.column ? Axis.vertical : Axis.horizontal;
...
abstract class TrackSize with Diagnosticable {
  ...
  @protected
  double _itemMinIntrinsicSizeOnAxis(RenderBox item, Axis axis, double crossAxisSize) {
    return axis == Axis.horizontal ? item.getMinIntrinsicWidth(crossAxisSize) : item.getMinIntrinsicHeight(crossAxisSize);
  }

  /// Helper function for determining the maximum intrinsic size of an item
  /// along the vertical or horizontal axis.
  @protected
  double _itemMaxIntrinsicSizeOnAxis(RenderBox item, Axis axis, double crossAxisSize) {
    return axis == Axis.horizontal ? item.getMaxIntrinsicWidth(crossAxisSize) : item.getMaxIntrinsicHeight(crossAxisSize);
  }
...
class FlexibleTrackSize extends TrackSize {
  ...
  const FlexibleTrackSize(this.flexFactor, {super.debugLabel}) : assert(flexFactor > 0);
...
bool trackSizeListsEqual(List<TrackSize> a, List<TrackSize> b) {
  ...
  return a.length == b.length && zip([a, b]).every((pair) => pair[0] == pair[1]);
}
