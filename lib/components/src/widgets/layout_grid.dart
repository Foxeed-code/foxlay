...
class AutoPlacement {
  ...
  static const rowSparse = AutoPlacement._(TrackType.row, AutoPlacementPacking.sparse);
  ...
  static const rowDense = AutoPlacement._(TrackType.row, AutoPlacementPacking.dense);

  /// Items are placed by filling each column in turn, adding new columns as
  /// necessary.
  static const columnSparse = AutoPlacement._(TrackType.column, AutoPlacementPacking.sparse);
  ...
  /// would fill in holes left by larger items.
  static const columnDense = AutoPlacement._(TrackType.column, AutoPlacementPacking.dense);
...
class LayoutGrid extends MultiChildRenderObjectWidget {
  LayoutGrid({
    ...
    assert(() {
      ...
      assert(parsedAreas.columnCount == columnSizes.length, 'areas.columnCount != columnSizes.length');
      assert(parsedAreas.rowCount == rowSizes.length, 'areas.rowCount != rowSizes.length');
      return true;
    }(), 'areas ');
  }
